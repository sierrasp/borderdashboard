...
export default class waitTimes {
	...
	async getCurrentWaitTimes() {
		 ...
		 */
		const lastWaitJson: { functionName: string; ports: number[] } = {
			functionName: 'getLastWaitTimes',
			ports: [this.portNum]
		};
		const averageWaitTimeJson: { functionName: string; ports: number[] } = {
			functionName: 'getAverageWaitTimes',
			ports: [this.portNum]
		};
		try {
			...
			const averageWaitTimes: { found: { avg: string; lane_type: number }[]; missing: number[] } =
				await (
			...
			const newDate = DateTime.fromISO(`${lastWaitTimes[0].daterecorded}`).setZone(
				'America/Tijuana'
			);
			console.log(newDate);
